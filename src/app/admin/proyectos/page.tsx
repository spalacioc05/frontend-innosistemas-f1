...
import { ProjectsServiceNew } from '@/services/projects';
...
export default function AdminProyectosPage() {
  ...
  useEffect(() => {
    CoursesService.list()
      .then((data) => setCourses(data.map((c) => ({ id: c.id, name: c.name }))))
      .catch(() => setCourses([]));
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const exists = await ProjectsServiceNew.existsByNameInCourse(courseId, name.trim());
      if (exists.exists) {
        setError('Ya existe un proyecto con este nombre en este curso');
        return;
      }

      await ProjectsServiceNew.createProject({ name: name.trim(), description: description.trim(), courseId });
